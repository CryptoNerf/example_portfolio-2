...
class PhotoFlipbook {
    ...
    /**
     * Вычисление оптимального размера книги
     */
    calculateBookSize() {
        const isMobile = window.innerWidth <= 768;
        const isTablet = window.innerWidth > 768 && window.innerWidth <= 1024;

        // Доступное пространство с учетом отступов и controls
        const availableWidth = window.innerWidth - 40; // 20px padding с каждой стороны
        const availableHeight = window.innerHeight - 220; // 80px top + 120px controls + margins

        let bookWidth, bookHeight;

        if (isMobile) {
            // На мобильных - одна страница (портретная)
            bookWidth = Math.min(availableWidth * 0.95, 500);
            bookHeight = Math.min(availableHeight * 0.9, bookWidth * 1.4); // Соотношение 1:1.4
        } else if (isTablet) {
            // На планшетах - разворот
            bookHeight = Math.min(availableHeight * 0.85, 600);
            bookWidth = Math.min(availableWidth * 0.9, bookHeight * 0.7);
        } else {
            // На десктопе - разворот с оптимальным размером
            bookHeight = Math.min(availableHeight * 0.9, 800);
            bookWidth = Math.min(availableWidth * 0.45, bookHeight * 0.7);
        }
    ...
    // Массив путей к фотографиям для Главы 2
    // Первая картинка - передняя обложка, последняя - задняя обложка
    const photos = [
        'img/8.jpg',   // Передняя обложка
        'img/9.jpg',
        'img/10.jpg',
        'img/11.jpg',
        'img/12.jpg',
        'img/8.jpg'    // Задняя обложка (дублируем первую картинку)
    ];
